...
**最后更新:** 2026-07-13(**W3 切片 H 装机反馈改造完成**:camera 插件页面内实时取景直拍 + 取景框放大居中,APK 重出 47.7MB;**复测第二轮修复**:release 包缺 `INTERNET` 权限致「开始分析」网络异常 —— flutter 模板只给 debug/profile manifest,main 已补,aapt 实证进包,APK 已再出;装机复测留用户)
...
- ⬜ 下一动作:手机 + 平板装机复测拍照全流程(真机在用户侧;实时取景/直拍/降级链 + 相册选图回归,release 包 API 走线上)。
