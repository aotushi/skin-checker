...
- 🟡 切片 E 安全加固(2026-07-14,用户指示「不带 token」):`app.ts` 共享层 —— `/analyze` per-IP 限流 10 次/分(实例级内存 Map,429 走既有 `{error}` 两端前端零改;IP 取法防伪造:Workers=CF-Connecting-IP、FC=XFF 末跳)+ CORS 收紧为白名单(`skin.9shi.cc` + 本地 dev;小程序/App/脚本无 Origin 不经此层不误伤)。本地 mock FC + wrangler dev 双跑全验(带图 200 / CORS 三组 / 10×400→2×429 / 首跳轮换伪造不可绕 / 换末跳独立桶)。**待重部署:FC ZIP 已重打(`dist/fc/fc.zip`)留用户控制台上传;Workers `wrangler deploy` 待同意**。配套建议(用户侧):FC 最大实例数封顶 + 云监控调用量告警 + 费用预算告警。
